...
function cssColorSet() {
  ...
  $(".color-indicator").each(function () {
    ...
    var c = color(
      colorList[counter],
      255 - COLOR_ADJ * colorList[counter],
      128
    );
    pop();
    $(this).css(
      "background-color",
      "rgb(" + c._getRed() + "," + c._getGreen() + "," + c._getBlue() + ")"
    );
    counter++;
  });
}
...
function stop() {
  if (
    //mode==Mode.acceleration || //加速中でもストップボタンを効かせるにはコメントアウト
    mode == Mode.constant
  ) {
    $("#start").css("display", "none");
...
function drawRoulette() {
  ...
  for (var i = 0; i < len; i++) {
    ...
    arc(
      ...
      RADIUS * 2,
      angleSum,
      angleSum + 2 * PI * probabilityList[i]
    );
  ...
  switch (mode) {
    ...
    case Mode.deceleration:
      if (speed > DECEL) {
        speed -=
          DECEL +
          getRandomInt(-DECEL_RAND_LEVEL, DECEL_RAND_LEVEL) *
            DECEL_RAND_MAGNITUDE;
      } else {
        speed = 0.0;
        mode = Mode.result;
      }
    ...
    case Mode.result:
      ...
      if (!resultDisplayed) {
        ...
        for (var i = 0; i < len; i++) {
          angleSum += probabilityList[i] * 2 * PI;
          if (
            (angleSum > (3 / 2) * PI && beforeAngleSum < (3 / 2) * PI) ||
            (angleSum > (7 / 2) * PI && beforeAngleSum < (7 / 2) * PI)
          ) {
            result = i;
            break;
          }
